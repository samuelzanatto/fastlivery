...
interface PWAHeaderProps {
  ...
  onBack?: () => void
  scrollBlur?: boolean
  menuType?: 'landing' | 'restaurant' // Nova prop para tipo do menu
...
export function PWAHeader({ 
  ...
  menuOnly = false,
  scrollBlur = false,
  menuType = 'restaurant', // Default para backward compatibility
  ...
  const handleMenuItemClick = (item: typeof menuItems[0]) => {
    if (item.label === 'Minha Conta') {
      // Close the menu sheet first
      setIsMenuOpen(false)
      // Then dispatch custom event to open profile sheet
      setTimeout(() => {
        window.dispatchEvent(new CustomEvent('openProfileSheet'))
      }, 100)
  ...
  const menuItems = menuType === 'landing' 
    ...
    : [
        // Menu para rotas públicas dos restaurantes (clientes)
        {
          icon: ShoppingBag,
          label: 'Meus Pedidos',
          href: '/pedidos',
                  ...
                  {session?.user ? (
                    ...
                    <div className="p-4 bg-gradient-to-r from-slate-50 to-slate-100/50 rounded-2xl text-center mb-6">
                      <div className="mb-3">
                        ...
                        <p className="text-slate-700 text-sm font-medium">
                          {menuType === 'landing' ? 'Explore o ZapLivery' : 'Faça login para acessar sua conta'}
                        </p>
                      </div>
                      <Button
