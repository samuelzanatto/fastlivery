...
export default function NewOrderDialog({ open, onOpenChange, onOrderCreated }: NewOrderDialogProps) {
  ...
  const handleSubmitOrder = useCallback(async () => {
    ...
    if (newOrder.orderType === 'dine_in' && !newOrder.tableNumber.trim()) {
      notify('error', 'Número da mesa é obrigatório para pedidos no local')
      return
    }

    if (newOrder.splitBill) {
      if (newOrder.people.length === 0) {
        notify('error', 'Adicione pelo menos uma pessoa para dividir a conta')
        return
      }

      const peopleWithoutNames = newOrder.people.filter(person => !person.name.trim())
      if (peopleWithoutNames.length > 0) {
        notify('error', 'Todos os nomes das pessoas devem ser preenchidos')
        return
      }

      const unassignedItems = getUnassignedItems()
      if (unassignedItems.length > 0) {
        notify('error', `${unassignedItems.length} itens não foram atribuídos a ninguém`)
        return
      }
    }
    ...
    try {
      if (newOrder.splitBill) {
        ...
        notify('success', `${newOrder.people.length} pedidos criados com sucesso (conta dividida)!`)
      } else {
  ...
  }, [newOrder, getUnassignedItems, onOpenChange, onOrderCreated])
  ...
  ), [handleSubmitOrder, newOrder.customerName, newOrder.items.length, onOpenChange])
