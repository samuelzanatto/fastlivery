...
export function ProductAdditionalsSelector({
  ...
  const loadAdditionals = useCallback(async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/additionals?restaurantId=${restaurantId}&limit=100`)
      if (!response.ok) throw new Error('Erro ao carregar adicionais')
      
      const data = await response.json()
      
      // Mapear items para manter compatibilidade
      const mappedAdditionals = data.additionals.map((additional: Additional) => ({
        ...additional,
      ...
      toast.error('Erro ao carregar adicionais do restaurante')
    } finally {
      setLoading(false)
    }
  }, [restaurantId])
        ...
        <CardContent>
          <div className="text-center py-8">
            ...
            <p className="text-gray-600 mb-4">
              Crie adicionais para o seu restaurante primeiro
            </p>
            <Button 
              variant="outline"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <Badge variant="outline" className="text-xs">
                      {additional.items.length} {additional.items.length === 1 ? 'item' : 'itens'}
                    </Badge>
