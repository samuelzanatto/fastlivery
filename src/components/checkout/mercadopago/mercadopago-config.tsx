...
export function MercadoPagoConfig({ businessId: _businessId }: MercadoPagoConfigProps) {
  ...
  const fetchConfig = async () => {
    try {
      ...
      if (result.success) {
        ...
        console.error('Erro ao buscar configuração:', result.error)
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validar formato básico dos tokens
      const isTestCredentials = formData.accessToken.startsWith('TEST-') && formData.publicKey.startsWith('TEST-')
      const isProdCredentials = formData.accessToken.startsWith('APP_USR-') && formData.publicKey.startsWith('APP_USR-')
      
      if (!isTestCredentials && !isProdCredentials) {
        setMessage({
          type: 'error',
          content: 'Formato de credenciais inválido. Use credenciais de produção (APP_USR-) ou teste (TEST-).'
        })
        setLoading(false)
        return
      }
      ...
      const isValid = await validateMercadoPagoCredentials(formData.accessToken, formData.publicKey)
      
      if (!isValid) {
        setMessage({
          type: 'error',
          content: 'Credenciais do Mercado Pago inválidas. Verifique se são credenciais de produção válidas.'
        })
        setLoading(false)
        return
      }

      const result = await updateMercadoPagoConfig(formData)

      if (result.success) {
        setMessage({
          type: 'success',
          content: 'Mercado Pago configurado com sucesso!'
        })
        ...
        setMessage({
          type: 'error',
          content: result.error || 'Erro ao configurar Mercado Pago'
        })
      }
    } catch {
      setMessage({
  ...
  const handleRemoveConfig = async () => {
    ...
    try {
      const result = await removeMercadoPagoConfig()

      if (result.success) {
        setMessage({
          type: 'success',
          content: 'Configuração do Mercado Pago removida com sucesso!'
        })
        await fetchConfig()
      } else {
        setMessage({
          type: 'error',
          content: result.error || 'Erro ao remover configuração'
        })
      }
    } catch {
      setMessage({
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-gray-600">Configure pagamentos PIX e cartão para sua empresa</p>
        </div>
        {config.configured && (
