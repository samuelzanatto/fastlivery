...
export class MercadoPagoService {
  ...
  constructor(restaurantAccessToken?: string) {
    // Credenciais específicas para cada funcionalidade
    const checkoutToken = restaurantAccessToken || process.env.MERCADOPAGO_ACCESS_TOKEN || '' // APP_USR- 
  ...
  async createPixPayment(data: MercadoPagoPaymentData) {
    ...
    try {
      ...
      const lastName = nameParts.slice(1).join(' ') || 'ZapLivery'
  ...
  const validNotificationUrl = notificationUrlEnv && /^https?:\/\//.test(notificationUrlEnv)
      ...
      const paymentData: {
        ...
        description: `Pedido ZapLivery - ${data.orderNumber}`,
  ...
  async createPaymentPreference(data: MercadoPagoPaymentData) {
    ...
    try {
      // Em desenvolvimento, priorizar NGROK_URL se disponível
      ...
      const preferenceData = {
        items: [
          {
            id: data.orderNumber,
            title: `Pedido ZapLivery - ${data.orderNumber}`,
  ...
  private shouldSetNotificationUrl(): boolean {
    const baseUrl = process.env.NGROK_URL || process.env.NEXTAUTH_URL || ''
  ...
  private mapMpStatus(status?: string): 'PENDING' | 'APPROVED' | 'REJECTED' | 'CANCELLED' {
 ...
 */
export async function createMercadoPagoService(restaurantId: string): Promise<MercadoPagoService> {
  ...
  // Criar o serviço passando as credenciais do restaurante
  return new MercadoPagoService(restaurant.mercadoPagoAccessToken)
}
