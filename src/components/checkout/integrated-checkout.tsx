...
import { usePaymentStatusPolling } from '@/hooks/payments/use-payment-status-polling'
...
import { MercadoPagoPaymentBrick } from '@/components/checkout/mercadopago/mercadopago-payment-brick'
import MercadoPagoStatusScreenBrick from '@/components/checkout/mercadopago/mercadopago-status-screen-brick'
...
function IntegratedCheckoutComponent({ businessSlug, businessStatus }: IntegratedCheckoutProps) {
  ...
  useEffect(() => {
    ...
    async function fetchKey() {
      ...
      pushDebug('fetchKey:start', { businessSlug })
      setLoadingPublicKey(true)
      try {
        const res = await fetch(`/api/business/${businessSlug}/public-key`)
    ...
    fetchKey()
    return () => { ignore = true }
  }, [businessSlug, debug, pushDebug])

  // Copiar código PIX
  const handlePixCopy = (_text: string) => {
  notify('success', 'Código PIX copiado! Cole no seu banco.')
  }
  ...
  notify('error', 'Pagamento não aprovado.')
      }
    }
  })
  ...
  const nextStep = useCallback(() => {
    switch (currentStep) {
      ...
      case 'address':
        if (!selectedAddress) {
          notify('error', 'Selecione um endereço de entrega')
          return
        }
      ...
      {debug && !publicKey && (
        <div className="mb-4 p-3 border border-amber-300 bg-amber-50 rounded text-amber-800 text-xs">
          ...
          <ul className="list-disc list-inside space-y-1">
            <li>Verifique registro em empresas: campos mercadoPagoPublicKey e mercadoPagoConfigured.</li>
            <li>Endpoint: /api/business/[slug]/public-key retornou null.</li>
        ...
        <div id="payment-brick-wrapper" className="-mx-2">
          <MercadoPagoPaymentBrick
            ...
            onPaymentError={(error) => {
              ...
              setPaymentState(prev => ({ ...prev, status: 'rejected' }))
              notify('error', error.message || 'Erro no pagamento')
            }}
          />
        </div>
        ) : null
        ...
        <div className="space-y-4">
          <MercadoPagoStatusScreenBrick
            ...
            onError={(error) => {
              ...
              notify('error', 'Erro ao carregar status do pagamento')
            }}
            backUrls={{
            ...
            <div className={`flex-shrink-0 border-t border-gray-100 bg-white px-6 ${currentStep === 'payment' ? 'py-0' : 'py-4'}`}>
              ...
              <div className="flex gap-4">
                ...
                {currentStep === 'cart' && (
                  <Button 
                    onClick={() => {
                      // Verificar se a empresa pode aceitar pedidos
                ...
                {currentStep === 'address' && (
                  <Button 
                    ...
                    disabled={!selectedAddress || !customerInfo.name || !customerInfo.email || !!(businessStatus && !businessStatus.canAcceptOrders)}
                  >
                    {businessStatus && !businessStatus.canAcceptOrders 
                      ? 'Empresa Fechada' 
                      : 'Continuar'
...
export const IntegratedCheckout = React.memo(IntegratedCheckoutComponent, (prevProps, nextProps) => {
  ...
  return (
    ...
    prevProps.businessStatus?.isOpen === nextProps.businessStatus?.isOpen &&
    prevProps.businessStatus?.canAcceptOrders === nextProps.businessStatus?.canAcceptOrders &&
    prevProps.businessStatus?.message === nextProps.businessStatus?.message &&
    prevProps.businessStatus?.nextChange === nextProps.businessStatus?.nextChange
  )
})