...
import { Check, AlertTriangle, Loader2, Building2, Truck } from 'lucide-react'
import { useDynamicPlans, type DynamicPlan } from '@/hooks/billing/use-dynamic-plans'
...
const isPopularPlan = (plan: DynamicPlan) => {
  const planId = plan.id.replace('delivery_', '').replace('supplier_', '')
  return planId === 'pro' || planId === 'growth'
}
...
export function DynamicPricingSection({ isExiting, onActionClick }: DynamicPricingSectionProps) {
  const { deliveryPlans, supplierPlans, isLoading, error, refetch } = useDynamicPlans()
  const [activeTab, setActiveTab] = useState<'delivery' | 'supplier'>('delivery')
  ...
  const currentPlans = activeTab === 'delivery' ? deliveryPlans : supplierPlans
    ...
    <section id="pricing" className="py-20">
      <div className="container mx-auto px-4">
        ...
        >
          ...
          {/* Tabs para separar tipos de planos */}
          <div className="flex justify-center mb-8">
            <div className="bg-slate-100 p-1 rounded-lg flex flex-row">
              <button
