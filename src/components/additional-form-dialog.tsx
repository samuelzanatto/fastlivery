...
export function AdditionalFormDialog({
  ...
  const removeItem = (index: number) => {
    if (fields.length > 1) {
      ...
      toast.error('Deve ter pelo menos um item')
    }
  }
  ...
  const onSubmit = async (data: AdditionalFormData) => {
    try {
      ...
      const url = isEditing ? `/api/additionals/${additional.id}` : '/api/additionals'
      ...
      if (!response.ok) {
        ...
        throw new Error(error.message || 'Erro ao salvar adicional')
      }

      toast.success(isEditing ? 'Adicional atualizado!' : 'Adicional criado!')
      setOpen(false)
      onSuccess()
      form.reset()
    } catch (error) {
      console.error('Erro ao salvar adicional:', error)
      toast.error(error instanceof Error ? error.message : 'Erro ao salvar adicional')
    } finally {
      setLoading(false)
    }
  }
